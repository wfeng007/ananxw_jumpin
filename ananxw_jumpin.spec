...
class AppBuilder:
    def __init__(self) -> None:
        ...
        self.entrance_scripts_a_=['main.py']
        # 打包生成exe的文件名称
        ...
        # 注意：如果工程本身不是包，工程下不要有__init__.py，可能会与实际包中的__init__.py冲突
        #   因为工程名很可能与包名一样，比如都叫ananxw_jumpin。同时又设置了pathex，两个都当做包去分析。
        self.scan_pkg_dirs_a_=[
            # '.', #spec或main当前目录作为分析目录？
            # self.this_spec_absdir, #如果工程目录同时也是包目录，spec就在包的根目录下时用；
            os.path.join(self.this_spec_absdir, self.appName) #指向包 ananxw_jumpin
        ]
        # 分析阶段，指定哪些文本文件直接打包到 name_coll_
        self.libs_datas_a_ = [ 
            # (os.path.join(self.this_spec_absdir, 'ananxw_jumpin'), 'ananxw_jumpin')
        ...
        # 导入动态模块 / 添加打包程序找不到的模块
        # 隐含导入，用了try块，条件导入，动态导入，甚至
        #   开发的包的_init_.py中是（from .submodule import Class）的形式
        #   静态分析都并不一定执行；
        # 要么指定隐含导入，要么在之后的拷贝处理的时候整个包拷贝
        # 隐含import指定会进行编译形成pyc.
        self.hiddenimports_a_ = [ 
            'ananxw_jumpin', # 似乎加不加无关。 只要pathex 有设定就行。
    ...
    def thisSpecAbsDir(self):
        ...
        # 使用spec当前目作为目录返回， 可用于，分析导入包的基础目录之一
